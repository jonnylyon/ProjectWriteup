...
\abstract{The development of major cities relies on the ability to build tall buildings that remain effective as practical places of work and leisure.  One very prominent consideration is the transportation of people up and down the building.  As buildings are built taller and taller, the superlinear increase in area required for elevator shafts has provoked the development of several new technologies and systems for efficient vertical transportation using fewer shafts, but these bring with them their own problems of `scheduling' (deciding how the elevator cars should behave).  This report explores one such concept, Two-Cars-in-One-Shaft (TCOS), whereby two separate cars share the same vertical shaft, with one always remaining above the other.  It goes on to discuss and develop several possible algorithms for scheduling these elevators, using `destination control', whereby users declare their destination at the time of first registering a call rather than when boarding the car, giving the system more information with which to make decisions.  A simulation tool is developed, using which all of the algorithms are evaluated and compared.  Ultimately, it is demonstrated that TCOS systems using the algorithms developed in this report provide significant space savings over traditional systems with similar performance.}
