...
\abstract{The development of major cities relies on the ability to build tall buildings that remain effective as practical places of work and leisure.  One very prominent consideration is the transportation of people up and down the building.  As buildings are built taller and taller, the superlinear increase in area required for elevator shafts has provoked the development of several new technologies and systems to allow efficient vertical transportation using fewer shafts.  However, these bring with them their own problems of `scheduling' (deciding how the elevator cars should behave).  This report explores one such concept -- Two-Cars-in-One-Shaft (TCOS) -- whereby two separate cars share the same vertical shaft, with one always remaining above the other.  It goes on to discuss and develop several possible algorithms for scheduling these elevators, using `destination control'.  Here, users declare their destination at the time of first registering a call rather than when boarding the car.  This gives the system more information with which to make decisions.  A simulation tool is developed, which is used to evaluate and compare all of the algorithms.  Ultimately, it is demonstrated that TCOS systems using the algorithms developed in this report provide significant space savings over traditional systems with similar performance.}
...
The elevator scheduling problem is one that is often completely overlooked by the average elevator user, but it is one of huge importance to the development of modern cities and population centres.  London, New York, Tokyo and indeed all of the world's major cities can be recognised by their dominating skylines, which come about from a need to make increasingly efficient use of limited land area by building upwards.  Without the development of safe and efficient elevators to transport people up and down these massive structures, the buildings simply could not operate effectively.

The nature and use of these buildings is such that required elevator capacity rises superlinearly with respect to the number of floors.  Due to the probably linear increases in both number of passengers, and in average journey times, we can imagine that in the general case required elevator capacity rises quadratically with the number of floors.  This means that continued innovation is required to improve the capacity that can be obtained with limited numbers of shafts.  Indeed, just two weeks before the submission of this report, it was announced that lift manufacturer Hitachi plan to install lifts capable of travelling at 72km/h (45mph) in a new skyscraper in Guangzhou, China, requiring technology to automatically alter air pressure inside the car to avoid causing pain to passengers \citep{BBC2014}.

One such innovation, marketed as TWIN by Thyssenkrupp, is the concept of having two cars in the same shaft (TCOS).  This brings benefits of having two separate cars serving passengers independently, using the same land space as just one traditional car.  However, it is a more complicated scheduling problem than the traditional case, due to the fact that cars have to interact with one another in order to avoid colliding.  While the lower car cannot visit the top floor, the upper car can visit the bottom floor due to the provision of a `garage' floor below the bottom floor at which the lower car can wait.

TCOS installations use Destination Control, a system in which passengers give their destination at the time of first registering a call for a lift.  This is in contrast to traditional systems where passengers initially state only their intended direction of travel.  This allows the lift controller to make more reliable and efficient decisions about which car should be allocated to serve each call.
...
The remainder of this report is structured in four sections.  An initial review of algorithms available in the existing literature is followed by the development of a simulation tool for testing algorithms for TCOS systems.  The next section presents and evaluates the performance of a number of allocation algorithms that can be used for TCOS systems, some of which are adapted from algorithms for traditional systems, and others of which are original.  Finally, conclusions of the project are presented along with suggestions for further work.
...
\begin{description}
	...
	\item[Interfloor traffic] In this traffic type, passengers travel from any floor in the building to any other floor (not necessary to/from floor 0) in any direction.  \citep{Pepyne1997, Collins1993Patent}
	\item[Lunchtime traffic] The lunchtime traffic type is a combination of the up-peak and down-peak types.  Because the lunch break is relatively short and probably overlaps, the situation can arise where passengers are going both up and down the building at once, but primarily either originating from or going to floor 0.  In the simplified case, all traffic either originates from or is destined for floor 0.  \citep{Bao1994, Brand2004, Smith2002, Pepyne1997}
\end{description}
...
In order to simplify the logic used in the implementation of the simulator, the following assumptions were made about the domain:
	\begin{itemize}
		...
		\item When a car arrives, the Passenger Groups wishing to board the car will do so strictly in the order in which they placed their calls.  The exception to this is when a Passenger Group is too large to be accommodated, in which case smaller groups from behind may `jump the queue'.
		\item All cars in an elevator system can call at all floors between the minimum and maximum floor specified.
		\item If the lowest floor is $l$ and the highest floor is $h$, then there are $(h-l)+1$ floors in total, represented by the integers in the range $[l, h]$.
		...
		\item Continuous allocation algorithms will only revoke previous call allocations when new calls come in (they will not generally monitor the state of the system except at times when they are invoked by new passengers making hall calls).
	...
	\centering
		\begin{tikzpicture}
			...
			\begin{axis}[width=125pt,
						 ...
						 x label style={at={(current axis.right of origin)},anchor=north, below=2mm}
						 ]
				
				
				\addplot+[domain={0:0.9},color=brown,mark=none,smooth,solid,thick] {6+((x^2)/(-2*0.5))};
				
				\addplot+[domain={0:sqrt(1/3)},color=green,mark=none,smooth,solid,thick] {5+((x^2)/(2*0.25))};
				\addplot+[domain={0:0.9},color=brown,mark=none,smooth,solid,thick] {5+((x^2)/(-2*0.5))};
				
				\addplot+[domain={0:sqrt(2/3)},color=green,mark=none,smooth,solid,thick] {4+((x^2)/(2*0.25))};
				\addplot+[domain={0:0.9},color=brown,mark=none,smooth,solid,thick] {4+((x^2)/(-2*0.5))};
				
				\addplot+[domain={0:0.9},color=green,mark=none,smooth,solid,thick] {3+((x^2)/(2*0.25))};
				\addplot+[domain={0:0.9},color=brown,mark=none,smooth,solid,thick] {3+((x^2)/(-2*0.5))};
				
				\addplot+[domain={0:0.9},color=green,mark=none,smooth,solid,thick] {2+((x^2)/(2*0.25))};
				\addplot+[domain={0:sqrt(2/3)},color=brown,mark=none,smooth,solid,thick] {2+((x^2)/(-2*0.5))};
				
				\addplot+[domain={0:0.9},color=green,mark=none,smooth,solid,thick] {1+((x^2)/(2*0.25))};
				\addplot+[domain={0:sqrt(1/3)},color=brown,mark=none,smooth,solid,thick] {1+((x^2)/(-2*0.5))};
				
				\addplot+[domain={0:0.9},color=green,mark=none,smooth,solid,thick] {0+((x^2)/(2*0.25))};
				
				\addplot+[blue,solid,mark=none,thick] coordinates{(0.9,1.62) (0.9,5.19)};
				
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] at (axis cs: 0,0) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] at (axis cs: 0,1) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] at (axis cs: 0,2) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] at (axis cs: 0,3) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] at (axis cs: 0,4) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] at (axis cs: 0,5) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] at (axis cs: 0,6) {};
				
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] at (axis cs: 0.577,0.667) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] at (axis cs: 0.577,1.667) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] at (axis cs: 0.577,2.667) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] at (axis cs: 0.577,3.667) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] at (axis cs: 0.577,4.667) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] at (axis cs: 0.577,5.667) {};
				
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] at (axis cs: 0.816,1.333) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] at (axis cs: 0.816,2.333) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] at (axis cs: 0.816,3.333) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] at (axis cs: 0.816,4.333) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] at (axis cs: 0.816,5.333) {};
				
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] at (axis cs: 0.9,2.19) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] at (axis cs: 0.9,3.19) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] at (axis cs: 0.9,4.19) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] at (axis cs: 0.9,5.19) {};
			\end{axis}
		\end{tikzpicture}
		\captionof{figure}{Paths through the continuous state space of an upward-moving elevator car, with the branching points shown in red.}
	...
	\centering
		\begin{tikzpicture}
			...
			\begin{axis}[width=125pt,
				...
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] (a) at (axis cs: 0,0) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] (b) at (axis cs: 0,1) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] (c) at (axis cs: 0,2) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] (d) at (axis cs: 0,3) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] (e) at (axis cs: 0,4) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] (f) at (axis cs: 0,5) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] (g) at (axis cs: 0,6) {};
				
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] (h) at (axis cs: 0.577,0.667) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] (i) at (axis cs: 0.577,1.667) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] (j) at (axis cs: 0.577,2.667) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] (k) at (axis cs: 0.577,3.667) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] (l) at (axis cs: 0.577,4.667) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] (m) at (axis cs: 0.577,5.667) {};
				
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] (n) at (axis cs: 0.816,1.333) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] (o) at (axis cs: 0.816,2.333) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] (p) at (axis cs: 0.816,3.333) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] (q) at (axis cs: 0.816,4.333) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] (r) at (axis cs: 0.816,5.333) {};
				
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] (s) at (axis cs: 0.9,2.19) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] (t) at (axis cs: 0.9,3.19) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] (u) at (axis cs: 0.9,4.19) {};
				\node[circle,fill=red,inner sep=0pt,minimum size=0.15cm] (v) at (axis cs: 0.9,5.19) {};
							
				\path[->, blue, thick]    (s)	edge  [bend right]	node	{}	(t)
								   (t)	edge  [bend right]	node	{}	(u)
								   (u)	edge  [bend right]	node	{}	(v);
				
				\path[->, brown, thick]   (h)	edge				node	{}	(b)
								   (i)	edge  [bend right]	node	{}	(c)
								   (j)	edge  [bend right]	node	{}	(d)
								   (k)	edge  [bend right]	node	{}	(e)
								   (l)	edge  [bend right]	node	{}	(f)
								   (m)	edge  [bend right]	node	{}	(g)
								   (n)	edge  				node	{}	(c)
								   (o)	edge  				node	{}	(d)
								   (p)	edge  				node	{}	(e)
								   (q)	edge  				node	{}	(f)
								   (r)	edge  				node	{}	(g)
								   (s)	edge  [bend left]	node	{}	(d)
								   (t)	edge  [bend left]	node	{}	(e)
								   (u)	edge  [bend left]	node	{}	(f)
								   (v)	edge  [bend left]	node	{}	(g);
								   
				\path[->, green, thick]   (a)		edge				node	{}	(h)
								   (b)	edge				node	{}	(i)
								   (c)	edge				node	{}	(j)
								   (d)	edge				node	{}	(k)
								   (e)	edge				node	{}	(l)
			...
			\end{axis}
		\end{tikzpicture}
		\captionof{figure}{The discretised state space of the same upward-moving elevator car.}
		\label{shaftstates}
	\end{minipage}
...
It should be noted that when a car decides to stop at a floor, it transitions directly to the stopped state of that floor, hence the multiple brown transition arrows to each floor in \autoref{shaftstates}.
...
There are a number of assumptions made about the TCOS case in addition to those made for the Core Elevator Simulator.
	\begin{enumerate}
		\item The separation of cars within a shaft is controlled by ensuring that no two cars may be at the same floor at the same time.
		\item When a car is moving from one floor ($a$) to the next ($b$), it is considered to occupy both floors at the same time.  Therefore the other car may not start moving towards floor $a$ until the first car has reached floor $b$.
...
The TCOS case introduced some complications to the constraints, which meant that decisions had to be made about the control logic in the Domain Model component.  I developed four strategies that could be used to avoid deadlock or collision between cars, which are described below.
...
This option further builds on the benefits of options 1 and 3, without contravening either the passenger expectation constraints or the conceptual logic architecture of the simulator.  For this reason, I chose to use this option (option 4) in the TCOS Elevator Simulator.
...
It was also necessary to change the behaviour of the cars so that, instead of simply stopping in situ and changing its action to Idle when there are no calls to serve, the car would move to its designated park floor at this stage before changing to a new action, named `Parked'.
...
The cars are all ranked by their respective values of the estimated cost function.  If the most preferred car rejects the allocation (see the notes in \autoref{tesoption4} for why this might happen) then the second most preferred car will be attempted, and so on.
...
The six parameters described previously all still apply for ETD: $Start$, $Stop$, $Travel$, $Reverse$, $Unload$, $Load$.  The SDF and Cost functions are adapted as follows:

\[ SDF(c, h) = \sum\limits_{g \in G(c)} (EST(g, c, G(c) \cup \left\{ h \right\}) - EST(g, c, G(c)))\]
\[ Cost(c, h) = SDF(c, h) + EST(h, c, G(c) \cup \left\{ h \right\}) \]
where $c$, $g$, $h$ and $G(c)$ are equivalent to the same variables in ETA and $EST(g,c,G(c))$ is the expected system time (from pressing the call button to exiting a car at the destination floor) of passenger group $g$ travelling in car $c$ with calls $G(c)$.
...
As part of the Core Elevator Simulator, a Manual Allocator was included which does not perform any calculation itself, but asks the user to choose which car the call should be allocated to.  In order to verify the behaviour of the three new Allocators, I have designed a small 5-minute passenger distribution which, when run using any of the new Allocators, causes the most important paths through the code to be executed.  For each algorithm, I have used the Manual Allocator and made the allocation decisions myself in the way that I expect the implemented Allocators to behave.  I facilitated this process by placing appropriate breakpoints in the code and examining the state of the system at the time of each hall call.  I have then compared my allocation decisions to those of the implemented Allocators in the same situations.  This method has allowed me to concentrate specifically on the behaviour of the new Allocator code without having to worry about the logic of the Domain Model, which has already been properly tested in chapters \ref{ceschapter} and \ref{teschapter}.  In all cases, the implemented Allocators were found to behave as described in \autoref{algdiscussion}.

\chapter{Evaluation of Allocation Algorithms}
\label{algevaluation}

There are a number of questions that I would like to answer in evaluating the various allocation algorithms:
\begin{itemize}
	\item Is there one specific algorithm that gives the best performances outright across all scenarios?  If not, which kinds of algorithm appear to be virtuous for each type of traffic flow?
	\item Do the computationally-simple algorithms, MOA and FZA, give performances that are competitive with the more complex ETA and ETD algorithms?
	\item In the case of ETA and ETD, do my extensions (Conflict-sensitivity and Load-sensitivity) give improved results over the basic na\"{i}ve versions?
	\item Is there any variant of FZA which provides reasonable performance in all traffic flow situations, such that cost could be saved by keeping one or more shafts as traditional shafts?
	\item Do TCOS systems give improvements over traditional systems with the same number of shafts?  If so, how many additional shafts are required for a traditional system to match the performance of a TCOS system?
\end{itemize}
...
As expected, ETA generally gave better AWT and ASWT results than ETD, while ETD outperformed ETA on AST and ASST.  However, there are some exceptions to this, for example in down-peak traffic where ETA was best by all measurements.  In all cases, the load-sensitive variants of ETD gave exactly the same allocations as the corresponding non-load-sensitive variants, suggesting that load-sensitivity does not add any value to ETD, while load-sensitivity tended to have a positive effect on ETA performance.  Indeed, the non-load-sensitive variants of ETA did notably poorly in up-peak traffic when measured by ASWT.
...
It can be seen from these results that, in \textit{every} traditional case tested, at least one additional shaft is required to match the performance of the 6-shaft TCOS system.  The number of additional shafts required depends on the traffic flow and the evaluation criterion used, but in the majority of cases 8 or more traditional shafts are required to match the performance of 6 TCOS shafts for the building and scenarios simulated.  In fact, regardless of which evaluation criterion is used, at least one of the traffic flows requires 8 traditional shafts to match the performance of 6 TCOS shafts.  A saving of 2 shafts may appear insignificant, considering that each TCOS shaft requires the installation of two cars which must come at a cost, but in a city the cost of land space is extremely significant.  Our simulations considered a building of 30 floors, and elevator cars with a capacity of 20 people.  A standard shaft for cars of similar capacity has an area of approximately 7.11 m\textsuperscript{2} \citep{StannahFactSheet}, which when counted over 30 floors takes up a total of 213.3 m\textsuperscript{2}.  The average renting price for office space in New York City is \$537.70 per square metre per year (July 2013) \citep{Patrick2013OfficeSpace}.  It follows, therefore, that a 30-storey building in New York could make a space saving worth almost \$115,000 per year for each shaft.  With this in mind, the potential benefit of installing 6 TCOS shafts instead of 8 traditional shafts can start to be appreciated.  The results also show the potential effectiveness of converting traditional shafts into TCOS shafts in an existing building if capacity is saturated, without needing to construct additional shafts.

\chapter{Conclusion and Further Work}

\section{Conclusion}

In this project I have collaboratively designed and developed a simulation tool (Core Elevator Simulator) that can be used to evaluate the performance of hall call allocation algorithms in traditional elevator systems.  This has involved adapting a positional state space with which to represent the location of cars within the shaft, as well as a behavioural one to represent the current actions of cars.  It was also necessary to consider and codify the logic used to control the immediate behaviour of an elevator.  In addition to these functional issues, the interaction between the user and the simulator has been considered and an appropriate system of configuration has been developed and implemented.  All elements of the Core Elevator Simulator have been tested, and the testing methods have been discussed.

I have independently extended the Core Elevator Simulator, creating an expanded tool capable of handling TCOS elevator shafts (TCOS Elevator Simulator).  Further work was required to understand the interaction between two cars sharing the same space for movement, the effect of these interactions on the control logic, and the changes required in order to adapt the control logic for the new case.  I have presented a number of potential solutions to these problems and discussed the benefits of each.  I have also given the reasoning behind choosing the one which was implemented.  Again, the behaviour of the TCOS Elevator Simulator has been thoroughly tested, and this has been discussed in the report.
...
Results of these evaluations have been presented, with a number of different evaluation criteria included, and suggestions are made as to how TCOS elevator operators might best apply the different algorithms to their respective circumstances.  Ultimately, it is found that the relative performance of the algorithms depends on the traffic flows under test and, unsurprisingly, that the best choice of algorithm depends on the evaluation criterion chosen.  However, the best all-round performances come from the Estimated Time of Arrival (ETA) and Estimated Time to Destination (ETD) algorithms, which benefit from several of my own improvements.
